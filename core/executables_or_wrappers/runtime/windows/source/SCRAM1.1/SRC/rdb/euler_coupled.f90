SUBROUTINE EULER_COUPLED(ns, nesp, dbound, grand, d, diam,j, kloc, alpha, LMD, Q_esp, N)
        ...
        DO jesp = 1, nesp
           ...
           IF (kloc(k) .LT. ns .AND. kloc(k) .GT. 1) THEN
              IF ( N_2evap_esp(kloc(k),jesp) .LT. 0D0 ) THEN
                 PRINT*,j, k,"->",kloc(k),grand(k),"N_2evap(k) negatif"
              ...
              IF ( N_2cond_esp(kloc(k),jesp) .LT. 0D0 ) THEN
                 PRINT*,j, k,"->",kloc(k),grand(k),"N_2cond(k) negatif"
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IF ( DABS (1D0 - (N_nouveau/N_ancien)) .GE. Eps_machine &
     ...
     PRINT*,j,"non conservation du nombre total !!"
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IF ( DABS (1D0 - (Q_nouveau/Q_ancien)) .GE. Eps_machine &
     ...
     PRINT*,j,"non conservation de la masse totale !!"
     PRINT*,j,"1 - Q_nouveau/Q_ancien ", 1D0 - Q_nouveau/Q_ancien   
     PRINT*,j, "Q_nouveau", Q_nouveau 
     PRINT*,j, "Q_ancien", Q_ancien
     STOP
  ENDIF
 ...
 !tests de limite minimum pour le nombre et le volume
  ...
  DO k=1, ns
     CALL COMPUTE_DENSITY(ns,nesp,Q_esp,LMD,k,rho(k))
  ENDDO
