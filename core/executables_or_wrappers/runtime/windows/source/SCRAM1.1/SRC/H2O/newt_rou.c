...
#define MAXITS 400 /* 50 */
#define STPMX 100.0
#define TINY 1.0e-20;
#define TOLF 1.0e-4 /* 1.0e-7 */
#define TOLMIN 1.0e-6   /* 1.0e-9 */
#define TOLX 1.0e-7    /*1.0e-10*/
	...
	void (*vecfunc)(int, float [], float []))
{
	...
	nn=n;
	nrfuncv=vecfunc;
	f=fmin_conflict(x); /* fvec[i] calculated */
	test=0.0;
	for (i=1;i<=n;i++){
	...
	for (its=1;its<=MAXITS;its++) {
		...
		lnsrch(n,xold,fold,g,p,x,&f,stpmax,check,fmin_conflict);
		test=0.0;
		for (i=1;i<=n;i++)
			if (fabs(fvec[i]) > test) test=fabs(fvec[i]);
		if (test < TOLF) {
