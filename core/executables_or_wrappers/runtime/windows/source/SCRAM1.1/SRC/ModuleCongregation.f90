...
!------------------------------------------------------------------------
    ...
    do j =(ICUT+1), N_size
      ...
      if(qn.gt.0.d0) then!skip empty cell
	do jesp=E1,E2
...
!------------------------------------------------------------------------
    implicit none
    integer :: j1,j2,j,s,jesp,k
    double precision ::c_number(N_size)
    double precision ::c_mass(N_size,N_aerosol)
    double precision ::rate_number(N_size)
    double precision ::rate_mass(N_size,N_aerosol)
    double precision :: mass_total_local, volume_total_local, rho_local
    
    do j = 1, N_size
      k = concentration_index(j,1)
      mass_total_local = 0.d0
      volume_total_local = 0.d0
      do s = 1, N_species
        jesp = List_species(s)
        rho_local = mass_density_aer(jesp)
        if (rho_local <= 0.d0) rho_local = fixed_density
        mass_total_local = mass_total_local + c_mass(j,jesp)
        volume_total_local = volume_total_local + c_mass(j,jesp) / rho_local
      enddo
      if (c_number(j) > TINYN .and. volume_total_local > 0.d0) then
        cell_mass_av(j) = mass_total_local / c_number(j)
        cell_diam_av(j) = (6.d0 * volume_total_local / c_number(j) / pi) ** (1.d0 / 3.d0)
      else
        cell_mass_av(j) = size_mass_av(k)
        cell_diam_av(j) = size_diam_av(k)
      endif
      if (cell_diam_av(j) < diam_bound(1)) cell_diam_av(j) = diam_bound(1)
    enddo
