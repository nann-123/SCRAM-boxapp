...
Module eRedistribution
  use ConservativeRemap12, only: moving_center_dualpivot
...
!------------------------------------------------------------------------
    ...
    ! SCRAM1.2: conservative kernel handles both growth and evaporation.
    if (conservative_remap12 .and. scheme >= 2) then
      call redistribution_size12()
      return
    endif
    Qesp=0.d0
    tmp_n=0.d0
  ...
  subroutine redistribution_size12()
    ! Pack explicit configured dry species, then water, to avoid assuming
    ! contiguous species IDs or dropping water in the 30-species box model.
    integer :: f,k,j,s,id
    double precision :: packed(N_sizebin,N_species+1), number(N_sizebin)
    double precision :: rho(N_species+1), pivot(N_sizebin), dry(N_sizebin)
    do s=1,N_species
      id=List_species(s)
      if (id==EH2O) error stop 'SCRAM1.2: water must not be in dry List_species'
      rho(s)=mass_density_aer(id)
      if (rho(s)<=0.d0) error stop 'SCRAM1.2: invalid species density'
    enddo
    rho(N_species+1)=mass_density_aer(EH2O)
    pivot=sqrt(diam_bound(1:N_sizebin)*diam_bound(2:N_sizebin+1))
    do f=1,N_fracmax
      do k=1,N_sizebin
        j=concentration_index_iv(k,f)
        number(k)=concentration_number(j)
        do s=1,N_species
          packed(k,s)=concentration_mass(j,List_species(s))
        enddo
        packed(k,N_species+1)=concentration_mass(j,EH2O)
      enddo
      ! Fail visibly on orphan carriers instead of silently deleting mass.
      ! This preserves a strict standalone number/species-mass contract.
      if (any(.not.ieee_is_finite(packed)) .or. any(.not.ieee_is_finite(number))) &
        error stop 'SCRAM1.2: nonfinite remap input'
      if (any(packed<0.d0) .or. any(number<0.d0)) error stop 'SCRAM1.2: negative remap input'
      do k=1,N_sizebin
        if ((number(k)<=0.d0 .and. sum(packed(k,:))>0.d0) .or. &
            (number(k)>0.d0 .and. sum(packed(k,1:N_species))<=0.d0)) &
          error stop 'SCRAM1.2: orphan mass/number before remap'
      enddo
      call moving_center_dualpivot(N_sizebin,N_species+1,N_species+1, &
        diam_bound,pivot,rho,packed,number,dry)
      do k=1,N_sizebin
        j=concentration_index_iv(k,f)
        concentration_number(j)=number(k)
        do s=1,N_species
          concentration_mass(j,List_species(s))=packed(k,s)
        enddo
        concentration_mass(j,EH2O)=packed(k,N_species+1)
      enddo
    enddo
    call compute_wet_mass_diameter(1,N_size,concentration_mass,concentration_number, &
      concentration_inti,wet_mass,wet_diameter,wet_volume,cell_diam_av)
  end subroutine redistribution_size12
