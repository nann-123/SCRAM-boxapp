...
!!-----------------------------------------------------------------------
module Coagulation
  use CoagulationNearest12, only: nearest_rate, nearest_event_rate_sum
  use Physicalbalance
  use Discretization 
...
!------------------------------------------------------------------------   
    ...
    ! SCRAM1.2: production nearest is state-dependent and coefficient-free.
    ! Preserve the existing optional weighted/cache prototypes and CSV audits.
    if (coeff_repartition_mode == COEFF_REPARTITION_DETERMINISTIC_NEAREST) then
      call nearest_rate(rate_number,rate_mass,c_number,c_mass)
      call coeff_reset_deltas()
      coeff_last_delta_number=rate_number
      coeff_last_delta_mass=rate_mass
      coeff_last_coag_mass_before=0.d0
      do i=1,N_species
        jesp=List_species(i)
        coeff_last_coag_mass_before=coeff_last_coag_mass_before+sum(c_mass(:,jesp))
      enddo
      coeff_last_coag_mass_after=coeff_last_coag_mass_before+sum(rate_mass)
      coeff_last_coag_mass_residual=sum(rate_mass)
      coeff_last_coag_number_before=sum(c_number)
      coeff_last_coag_number_after=sum(c_number)+sum(rate_number)
      coeff_last_coag_event_rate_sum=nearest_event_rate_sum
      coeff_last_coag_number_residual=sum(rate_number)+nearest_event_rate_sum
      return
    endif
