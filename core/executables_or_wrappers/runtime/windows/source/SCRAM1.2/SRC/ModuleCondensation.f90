...
!------------------------------------------------------------------------
    ...
    do j = 1,N_size! Reassigned distribution by mass of each species
      ! SCRAM1.2: local physical diameter; no CAM state/interface required.
      if(.not.ieee_is_finite(wet_diameter(j)).or.wet_diameter(j)<=0.d0) &
        error stop 'SCRAM SULFDYN: invalid wet diameter'
    ...
    do j = 1,N_size! Reassigned distribution by mass of each species
      ! Empty/zero-accommodation aerosol populations have a zero sink.
      tmp=0.d0
      if(ce_kernal_coef_tot>0.d0.and.dtx>0.d0) then
	...
	tmp=0.d0
      ...
      N1(j)=N(j)
      dqdt(j,jesp)=0.d0
      if(dtx>0.d0) dqdt(j,jesp)=tmp/dtx!for redistribution
    enddo
...
!------------------------------------------------------------------------
    ...
    ! SCRAM1.2: initialize the full standalone storage, including water.
    Kelvin_effect=1.d0
    ce_kernel=0.d0;ce_kernal_coef_i=0.d0
    qext=q;init_bulk_gas=c_gas;qinti=0.d0
    surface_equilibrium_conc=0.d0
    if(.not.ieee_is_finite(Wet_diam).or.Wet_diam<=0.d0.or.qn<=TINYN) return
...
!     Aerosol wet density in � g.� m -3
    ...
    ! Total mass concentration / (number concentration * particle volume).
    ! ug um-3 -> kg m-3 is 1e9; this is a standalone dimensional correction.
    rhop =rhop/(qn*Wet_volume)
    rhop_tmp = rhop * 1.D9
