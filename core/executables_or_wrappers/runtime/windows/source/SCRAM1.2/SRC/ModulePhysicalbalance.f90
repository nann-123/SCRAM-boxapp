...
!------------------------------------------------------------------------   
    ...
    ! SCRAM1.2: never clamp diameter by changing prognostic number in modern
    ! mode. Only the conservative remap may move population between bins.
    if(redistribution_method.gt.2 .and. .not.conservative_remap12) then
      do j= 1, N_size
	k=concentration_index(j, 1)!size bins
	volum_cell=0.d0
	do s= 1, N_species
