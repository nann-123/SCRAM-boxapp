...
PROGRAM SCRAM
  ...
  call cond_surr_open()   !! Dataset I/O (no-op unless SCRAM_COND_SURR_IO=1)
  ...
  call system('command -v setterm >/dev/null 2>&1 && setterm -cursor off')
  ...
  do while (equilibrium_time.lt.final_time)
	...
	if (with_cond+tag_nucl+with_coag.eq.0) then
	  final_sub_time  = initial_time_splitting + timestep_splitting!pure emission
		...
		write(vchar,'(i3)') n
	 call system('echo -e "\b\b\b\b\b\b\b\b\c"; echo -e " '//trim(adjustl(vchar))//'%\c"')
      enddo
  ...
  print *, ''
  call system('command -v setterm >/dev/null 2>&1 && setterm -cursor on')

  ! Check mass conservation
  call mass_conservation(concentration_mass,concentration_number,&
  concentration_gas, total_mass)

  call cond_surr_close()  ! Flush dataset file if open
