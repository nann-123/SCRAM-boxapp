...
!!    Updated Oct. 16 2015
!!    Add new redistribution method with Kelvin effect
!!-----------------------------------------------------------------------
...
!------------------------------------------------------------------------   
    ...
    double precision rhop_tmp,emw_tmp,wet_diam,volume12,mass12
...
!!     ******zero init
  ...
  !compute only c/e coefficients
	...
	if (aerosol_species_interact(jesp).GT.0) then
      ...
      ! SCRAM1.2: per-particle wet density, without changing organic closure.
      volume12=wet_diam**3*cst_pi6
      mass12=sum(concentration_mass(j,:))
      rhop_tmp=1400.d0
      if(volume12>0.d0.and.concentration_number(j)>TINYN.and.mass12>0.d0) &
        rhop_tmp=mass12/(concentration_number(j)*volume12)*1.d9
	    call COMPUTE_KELVIN_COEFFICIENT(&
		      Temperature,&          ! temperature (Kelvin)
		      emw_tmp,&       ! ext mol weight (g.mol-1)
		      surface_tension(jesp),&   ! surface tension (N.m-1) from INC
		      wet_diam,&         ! wet aero diameter (�m)
	    ...
	    if(Kelvin_effect(j,jesp).le.1.d0) Kelvin_effect(j,jesp)=1.01
