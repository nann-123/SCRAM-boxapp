...
Module Adaptstep
  use CoagulationNearest12, only: scram_coag_flux_dt
  use Discretization
  use Initialization
  use Condensation
  use Congregation
...
!    ******Dynamic time loop
	...
	if (sulfate_computation.eq.1) then
	  call SULFDYN(concentration_mass_tmp,concentration_mass,concentration_number_tmp,&
	    ...
	    if(nucl_model.eq.5) then
	      !in case of two sulfate species validation test
	      call approxim_timestep(dqdt,sub_timestep_splitting)
	    endif
	endif
      endif
         	  
      if(ICUT.eq.N_size.AND.tag_coag.eq.0.AND.tag_nucl.eq.0) then
	sub_timestep_splitting=final_sub_time-current_sub_time
	current_sub_time=final_sub_time
      elseif (solver.eq.0) then
			  ! euler solver
	call cond_surr_record_before(sub_timestep_splitting, current_sub_time)
	...
	call mass_conservation(concentration_mass,concentration_number,&
      ...
      !!print*,time_p,sub_timestep_splitting
...
!     Coagulation time step.
    ...
    if (with_coag.eq.1) then
      ...
      tag_cond=0
      tag_nucl=0
      ! SCRAM1.2: estimate timescales from unrestricted rates, not stale dt.
      scram_coag_flux_dt=0.d0
...
!     q2: aerosol mass concentration of second order evaluation(�g/m^3)
...
!     -- INPUT VARIABLES
!
!     dqdt: aerosol mass concentration of first order evaluation(�g/m^3)
!
!     -- OUTPUT VARIABLES
!
...
!------------------------------------------------------------------------
    ...
    ! SCRAM1.2: each derivative uses its actual Euler/ETR stage budget.
    ! Outside coagulation, disable the limiter (timescale estimates included).
    scram_coag_flux_dt=0.d0
    if(tag_coag.eq.1) scram_coag_flux_dt=sub_timestep_splitting
...
!------------------------------------------------------------------------
    ...
    integer::j,jesp!s is the species index of ESO4
    double precision:: dqdt(N_size,N_aerosol)
    ...
    double precision, parameter :: Jd1_cap = 1.d6
  !for condensation
    ! SCRAM1.2: each derivative uses its actual Euler/ETR stage budget.
    ! Outside coagulation, disable the limiter (timescale estimates included).
    scram_coag_flux_dt=0.d0
...
! 	enddo
    ...
    if (Jd1(j,jesp).ne.0.d0) Jd1(j,jesp) = DSIGN(DMIN1(DABS(Jd1(j,jesp)), Jd1_cap), Jd1(j,jesp))
	enddo
      endif
    enddo
...
!     ******4. second evaluation
    ...
    do j = 1 , N_size
      ...
      do jesp = E1, E2
    den = 1.d0- Gamma*Jd1(j,jesp) *sub_timestep_splitting
    if (DABS(den) .lt. 1.d-12) den = DSIGN(1.d-12, den)
    k2(j,jesp) = (k2(j,jesp)*sub_timestep_splitting- Gamma*sub_timestep_splitting&
        *(Jdn(j,jesp)+Jd1(j,jesp))*k1(j,jesp)) / den
    if (IsNaN(k2(j,jesp))) k2(j,jesp) = k1(j,jesp)
      enddo
    enddo
    ...
    do j = 1 , N_size
      ...
      do jesp = G1, G2
        prev_q2 = q2(j,jesp)
        q2(j,jesp)=DMAX1(0.01d0*prev_q2,(prev_q2+0.5d0*(k1(j,jesp)+k2(j,jesp))) )
        tmp = q2(j,jesp) - prev_q2
        if (IsNaN(q2(j,jesp)) .or. IsNaN(tmp)) then
          q2(j,jesp) = prev_q2
          tmp = 0.0d0
        end if
        dqdt(j,jesp)=tmp/sub_timestep_splitting
