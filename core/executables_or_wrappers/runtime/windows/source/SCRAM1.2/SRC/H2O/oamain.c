...
****************************************************************************/
void oamain_(float *tempk, float *rh, float *worg, float *gasorg, float *partorg, float *mwaom_mix_loc,
	     float *lwc, float *protonconc, float *Organion, float *DeltaLWC, int *tboaflag, int* ioligo,
...
void h2o_localeq_(float *tempk, float *rh, float *worg, float *gasorg, 
		  float *mwaom_loc, float *lwc, float *protonconc, int *tboaflag,
		  int* ioligo,double *qsatref, double *tsatref, double *kpartref,
		  double *drh, double *dhvap, int *ithermo, float *cPOA)
{
  int i, j, ieq;                   /* counter */

  extern void amain_loc (float gasa[], float aeroa[], int* ioligo);
  extern void bmain_loc (float gasb[], float aerob[], float MWaom, float MO, float cPOA, int* ioligo);
  ...
  float aerob[(NBSP+1)];    
               ...
               /* for newt: initial guesses of Ai also contain final output */
               /* for direct solution: initial input Ai */
  float gasa[(NAMOL+1)];            /* type B gas-phase concentration */
  float gasb[(NBSP+1)];            /* type B gas-phase concentration */
  float tmp;         /* tmp var for partorg, gasorg type casted from double */
  float MWaom;
  float MO,cPOA_loc;
  ...
  amain_loc(gasa, aeroa, ioligo);

  for (i = 0; i < NAMOL; i ++) 
    {
      gasorg[i] = ((float)(gasa[i+1]));
    }
  ...
  for (i = (NAMOL+1); i <= (NAMOL + NBSP); i ++)
    {
      gasb[i-NAMOL] = gasorg[i - 1];
      aerob[i-NAMOL] = worg[i - 1];
      MO+=worg[i - 1];
    }

  bmain_loc(gasb, aerob, MWaom, MO, cPOA_loc, ioligo);
  for (i = NAMOL; i < (NAMOL + NBSP); i ++) 
    {
      gasorg[i] = ((float)(gasb[i-NAMOL+1]));
    }
