...
!------------------------------------------------------------------------
    ...
    integer :: kind_grid, mode_status
    character(len=64) :: remap_mode12
    double precision,dimension(:), allocatable::diameter

      !initial pointer
    call pointer_initial()
    ! SCRAM1.2: do not reinterpret old numeric scheme IDs. Default modern
    ! remap overrides size schemes >=2; explicit legacy restores old behavior.
    call get_environment_variable('SCRAM_REDISTRIBUTION_MODE', remap_mode12, status=mode_status)
    conservative_remap12 = .true.
    if (mode_status == 0) then
      select case(trim(remap_mode12))
      case('legacy','LEGACY')
        conservative_remap12 = .false.
      case('moving_center_dualpivot','MOVING_CENTER_DUALPIVOT','')
        conservative_remap12 = .true.
      case default
        error stop 'SCRAM1.2: unknown SCRAM_REDISTRIBUTION_MODE'
      end select
    endif
