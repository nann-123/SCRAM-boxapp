...
contains
  ...
  subroutine coeff_bootstrap_mode()
    implicit none
  ...
  end subroutine coeff_bootstrap_mode
  ...
  subroutine coeff_boxmodel_init()
    ...
    ! SCRAMBoxApp port fix (was commit 45de542 in the SCRAM1.1 tree, re-merged for 1.2):
    ! upstream 1.2 calls `execute_command_line('mkdir -p ...')` unconditionally, which is a
    ! POSIX-only command — on Windows `cmd` would end up creating a literal '-p' directory and
    ! fail to build the nested path. coeff_make_dir picks the right command per platform.
  ...
  ! SCRAMBoxApp port helper: create a directory through the shell. The command
  ! differs by platform — cmd's mkdir on Windows, mkdir -p on POSIX.
  ...
  subroutine coeff_parse_repartition_mode(mode_text)
    ...
    ! SCRAM1.2: accept short alias, with production state-dependent port.
    case ('NEAREST', 'DETERMINISTIC_NEAREST', 'DETERMINISTIC_BOX', 'DETERMINISTIC_BOX_NEAREST', 'COAG_TARGET_NEAREST')
      coeff_repartition_mode = COEFF_REPARTITION_DETERMINISTIC_NEAREST
      coeff_repartition_mode_name = 'COAG_TARGET_NEAREST'
    ...
    case default
      ! SCRAM1.2: a typo must not silently select unrelated coefficient I/O.
      error stop 'SCRAM1.2: unknown SCRAM_COEFF_REPARTITION_MODE'
    end select
