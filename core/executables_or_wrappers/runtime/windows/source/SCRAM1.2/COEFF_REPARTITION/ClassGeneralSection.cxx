...
namespace CoefficientRepartition
{
  ...
  void ClassGeneralSection::Init(const string type)
  {
   ...
   if(Nfrac_!=0)//calculate composition sections automatically!!ops.Exists("autofraction")
   {
     ...
     vector<double> frac_bound(Nfrac_ + 1);
	  ...
	  for (int i = 0; i < int(vtmp.size()); i++)
	    {
	      ...
	      //cout<<"c "<<c<<" i "<<i<<" fraction_0 "<<fraction[0]<<" fraction_1 "<<fraction[1]<<endl;
		...
		//cout<<" c "<<c<<" i "<<i<<" j "<<j<<" fraction_index "<<fraction_index_(c)(i)(j)<<endl;
	      }
	    }
	}
    }
    cout << "End of init" << endl;//TMP
    ops.Close();
  }
  ...
  {//fund the worst problem of negtive fraction particles
	...
	frac_avalibe-=(frac-fraction(i)(0));
	 ...
	 //cout<<i<<"frac="<<frac<<" frac_limt="<<frac_limt<<" frac_avalibe="<<frac_avalibe<<endl;
	 
         double frac_rest(double(1));
// 	//cout<<"check 4.2 i= "<<i<<" frac="<<frac<<" frac_rest "<<frac_rest<<endl;
//         for (int j = 1; j < fraction_index(i).GetSize(); j++)
//           {//this never happend because j always =0! I think this is designed for group species j is the number of species
// 	    //within the group
// 	    //cout<<"check 4.3 j="<<j<<"g "<<fraction_index(i).GetSize()<<endl;
//             int s = fraction_index(i)(j);
//             double frac_div = frac_rest * drand48();
//             p.fraction_(s) = frac * frac_div;
//             frac_rest -= frac_div;
//             has_fraction(s) = true;
//             count_has_fraction++;
//           }
        ...
        //has_fraction(s) = true;
        //count_has_fraction++;
	//cout<<"count_has_fraction:"<<count_has_fraction<<endl;
      }
  ...
  for(int s=0;s<this->Ns_;s++)
  {
    if(p.fraction_(s)>=fraction(s)(0)&&p.fraction_(s)<=fraction(s)(1))
    {
     ...
     if (count_has_fraction < this->Ns_|| frac_sum!=1.0)
       {
//         Vector1I index(this->Ns_ - count_has_fraction);
//         int j(0);
//         for (int s = 0; s < this->Ns_; s++)
//           if (! has_fraction(s))
//             index(j++) = s;
// 
//         double frac_rest = double(1) - frac_sum;
// 
//         for (int j = 1; j < index.GetSize(); j++)
//           {
//             int s = index(j);
//             double frac_div = frac_rest * drand48();
//             p.fraction_(index(j)) = frac_div;
//             frac_rest -= frac_div;
//           }
// 
//         p.fraction_(index(0)) = frac_rest;
  ...
  inline bool ClassGeneralSection::has_particle(const ClassParticle &p) const
  {
    ...
    //cout<<size_bin_<<" p.m="<<p.mass_<<" mass(0)="<<mass(0)<<" mass(1)="<<mass(1)<<endl;
    //cout<<size_bin_<<"Nb"<<Nb_<<endl;
    if (size_bin_ == (Nb_-1))
      {
        // If on last bin, consider particle to be in section even if
        // its mass exceeds the upper bound, thus not checked hereafter.
	//cout<<"xxx"<<size_bin_<<" p.m="<<p.mass_<<" mass(0)="<<mass(0)<<" mass(1)="<<mass(1)<<endl;
        if (p.mass_ < mass(0))
          return false;
      }
    else
    {
      //cout<<"mass="<<p.mass_<<" m0="<<mass(0)<<" m1="<<mass(1)<<endl;
    ...
    for (int i = 0; i < fraction_index.GetSize(); i++)
      {
        double frac_sum(0);
        for (int j = 0; j < fraction_index(i).GetSize(); j++)
	{// j=0 always 1 species for each group
	  //cout<<p.fraction_(fraction_index(i)(j))<<" "<<fraction_index(i)(j)<<endl;
          frac_sum += p.fraction_(fraction_index(i)(j));
	}

	//cout<<i<<" frac_sum="<<frac_sum<<" frac(0)="<<fraction(i)(0)<<" frac(1)="<<fraction(i)(1)<<endl;
