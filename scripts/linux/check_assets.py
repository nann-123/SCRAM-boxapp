...
退出码：0 = 无发现；1 = 有告警（过期/疑似缺字/字体缺失）；2 = 引用缺失或渲染失败。
"""
...
def check_staleness() -> None:
    ...
    if not app_t or not shot_t:
        warnings.append("无法读取 app/ 或 docs/screenshots/ 的最后改动时间（浅克隆？）")
        return
    if app_t > shot_t:
        warnings.append(
            f"截图已过期：app/ 最后改动 {app_t} 晚于 docs/screenshots/ 的 {shot_t}"
            "（devkit §7：改了 GUI 布局/按钮/文字/结果页就必须重生成截图）")
...
def check_fonts() -> None:
    ...
    if is_tofu(WINDOWS_FONT if WINDOWS_FONT in families else None):
        if not any(not is_tofu(f) for f in sorted(families)[:200]):
            ...
            warnings.append("默认字体渲染中文为方框；存在可用中文字体，生成脚本需要设置回退链")
    ...
    for directory in ASSET_DIRS:
        for path in sorted(directory.rglob("*.png")):
            ...
            try:
                ...
                warnings.append(f"{path.name}: 无法解析尺寸")
                continue
            ...
            if ratio < 0.015:
                ...
                warnings.append(f"{where}: 信息密度异常低（{ratio:.4f} B/px）→ 疑似文字渲染为方框或图为空白"
                                "（健康的 GUI 截图约 0.02–0.18 B/px，结果图约 0.03–0.07 B/px）")
...
def compare_with_render() -> None:
    ...
    for fresh in sorted(out.glob("*.png")):
        ...
        if fresh_d > old_d * 2.5:
            warnings.append(f"{fresh.name}: 仓库内资产密度 {old_d:.4f} 仅为新渲染 {fresh_d:.4f} 的 "
